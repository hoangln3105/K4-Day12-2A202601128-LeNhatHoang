...
class ShutdownGuard:
    ...
    def start_draining(self, signum=None, frame=None) -> None:
        ...
        # đè handler của uvicorn — thứ thật sự dừng server. Không gọi lại
        # nó thì app bật cờ "đang tắt" rồi chạy tiếp mãi, cho tới khi
        # orchestrator hết kiên nhẫn và SIGKILL.
        previous = self._previous.get(signum)
        if callable(previous):
            previous(signum, frame)
    ...
    def arm(self) -> None:
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            # Nhớ handler cũ TRƯỚC khi ghi đè, nếu không thì mất luôn tham
            # chiếu tới handler dừng server của uvicorn.
