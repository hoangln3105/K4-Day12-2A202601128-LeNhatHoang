...
def chat(
    ...
    """
    # Chặn TRƯỚC khi gọi LLM: tiền mất ở bước generate_reply, chặn sau khi
    # đã gọi thì vừa trả tiền vừa trả lỗi.
    ...
    return {
        "reply": result["text"],
        "client_id": client_id,
        # history đọc TRƯỚC khi ghi 2 lượt mới, nên đây là số lượt cũ
        "turns_before": len(history),
        "usd_cost": result["usd_cost"],
        "usage": {
            "prompt": result["prompt_tokens"],
